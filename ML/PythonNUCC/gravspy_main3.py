#Script by CIERA Intern Group, 7/22/16
    ...
    for i in range(N):
        ...
        if images['type'][i] == 'G': #check if golden set image
            ...
            if indicator1 == 0: #if image is not retired
                ...
                dec_matrix[0,i], class_matrix[0,i] = decider(pp_matrix, ML_dec, t, R_lim, num_annotators,C) #make decisions for each image in batch
...
Updating the Confusion Matrices for Test Data and Promotion
    ...
    print(conf_matrices,PP_matrices,retired_images)            

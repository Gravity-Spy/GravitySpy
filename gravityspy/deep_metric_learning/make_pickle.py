...
import datetime
import optparse
from GS_utils import my_save_dataset_test, my_read_image2, my_save_dataset_stratified


'''
By Sara Bahaadini, Neda Rohani, and  Scott Coughlin.
...
def parse_commandline():
    ...
    parser.add_option("--test-flag", type=int, default=1, help="test flag to show the pickles are for unlabeled glitches")
...
def main():
    ...
    imgs_1, label1, name1, classes = my_read_image2(save_address, dataset_path, 'spectrogram_2.0.png', resolution, simi_flag)
    ...
    for idx, fi in enumerate(name1):
            ...
            path2 = dataset_path + '/' + classes[label1[idx]] + '/' + fi + '_spectrogram_1.0.png'
            ...
            file5 = rescale(file5, [resolution,resolution], mode='constant')
            file2 = rescale(file2, [resolution, resolution], mode='constant')
            file4 = rescale(file4, [resolution, resolution], mode='constant')
    ...
    if test_flag:
        ...
        my_save_dataset_stratified(save_address, imgs_1_fl, label1, name1, '_1.0_')
        my_save_dataset_stratified(save_address, imgs_4_fl, label4, name4, '_4.0_')
        my_save_dataset_stratified(save_address, imgs_2_fl, label2, name2, '_2.0_')
        my_save_dataset_stratified(save_address, imgs_5_fl, label5, name5, '_5.0_')

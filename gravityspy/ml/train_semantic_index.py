...
from GS_utils import (cosine_distance, canberra_distance,
...
def make_model(data,
    ...
    Parameters:
        ...
        distance_method (`str`, optional):
            What distance function would you like to be optimized
    ...
    if distance_method=='cosine':
        ...
        distance = Lambda(canberra_distance,
                          output_shape=eucl_dist_output_shape)(
                              [processed_a, processed_b]
                          )
...
def generate_pairs(image_data, same_pair_labels, diff_pair_labels, batch_size):
    # select random same and different pairs
    random_same_idx = numpy.random.choice(len(same_pair_labels), batch_size)
    random_diff_idx = numpy.random.choice(len(diff_pair_labels), batch_size)

    # Alternate same and diff pairs
    stacked_idxs = numpy.zeros((batch_size*2, 3))
    stacked_idxs[::2] = diff_pair_labels[random_diff_idx]
    stacked_idxs[1::2] = same_pair_labels[random_same_idx]
    stacked_idxs = stacked_idxs.astype(numpy.int32)

    # Obtain appropriate indices from image_data array
    pairs1 = image_data[stacked_idxs[:, 0]]
    pairs2 = image_data[stacked_idxs[:, 1]]

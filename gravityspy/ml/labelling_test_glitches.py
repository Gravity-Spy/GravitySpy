...
def get_multiview_feature_space(image_data, semantic_model_name,
    ...
    if order_orig:
        test_set_unlabelled_x_1 = numpy.vstack(image_data[one_second_images].iloc[0]).reshape(reshape_order)
        test_set_unlabelled_x_2 = numpy.vstack(image_data[two_second_images].iloc[0]).reshape(reshape_order)
        test_set_unlabelled_x_3 = numpy.vstack(image_data[four_second_images].iloc[0]).reshape(reshape_order)
        test_set_unlabelled_x_4 = numpy.vstack(image_data[half_second_images].iloc[0]).reshape(reshape_order)
    else:
